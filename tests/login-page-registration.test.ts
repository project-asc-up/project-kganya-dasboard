...
test("login page uses Clerk sign-in and hides in-app registration UI", () => {
  ...
  assert.doesNotMatch(pageSource, /SignUpButton/);
  assert.match(pageSource, /Show/);
  assert.match(pageSource, /UserButton/);
  assert.doesNotMatch(pageSource, /Create an Account/);
  assert.doesNotMatch(pageSource, /Clerk authentication is enabled/);
  assert.doesNotMatch(pageSource, /Use Clerk secure authentication to access the workspace/);
  assert.doesNotMatch(pageSource, /Account verification and password security are handled by Clerk/);
