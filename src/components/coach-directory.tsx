  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_280px] items-start">
          <div className="max-h-[72vh] space-y-4 overflow-y-auto pr-2 xl:order-1 order-2">
            {grouped.length === 0 ? (
              <Card>
          ...
          <div className="flex flex-col gap-2 max-h-[72vh] overflow-y-auto xl:sticky xl:top-6 pr-2 xl:order-2 order-1">
            <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-[var(--color-text-muted)] mb-2 px-1">
              Filter by Faculty
            </h3>
            <Button
              variant={facultyFilter === "all" ? "primary" : "secondary"}
              onClick={() => setFacultyFilter("all")}
              className="justify-start px-4 h-10 w-full"
            >
              All faculties
            </Button>
            {facultyOptions.map((faculty) => (
              <Button
                ...
                className="justify-start px-4 h-10 w-full text-left"
              >
                {displayFacultyName(faculty.name)}
              </Button>
            ))}
