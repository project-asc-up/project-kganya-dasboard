...
export async function unsuspendUserAction(
  ...
  try {
    ...
    const authz = await requireSuperAdmin();
    const userId = String(formData.get("userId") ?? "");
    const reason = String(formData.get("reason") ?? "").trim(); // Optional note
    ...
    await prisma.userModerationRecord.create({
      data: {
        ...
        reason: reason || "No administrative note provided.",
      },
    });

    // Notify base44 agent to send unsuspend email
    if (targetEmail) {
      try {
        const noteSection = reason ? ` Administrative note: "${reason}"` : "";
        const messageText = `Please send an email to ${targetEmail} informing them that their account access has been restored (unsuspended).${noteSection}`;
        await fetch(
          "https://app.base44.com/api/agents/6a00597ec92cb8615f50b66d/conversations/6a005983777455158d138471/messages",
          {
            method: "POST",
        ...
        console.error("Failed to notify base44 agent of user unsuspension:", apiError);
      }
    }
...
export async function unbanUserAction(
  ...
  try {
    ...
    // Notify base44 agent to send unban email
    if (targetEmail) {
      try {
        const noteSection = reason ? ` Administrative note: "${reason}"` : "";
        const messageText = `Please send an email to ${targetEmail} informing them that their account access has been restored (unbanned).${noteSection}`;
        await fetch(
          "https://app.base44.com/api/agents/6a00597ec92cb8615f50b66d/conversations/6a005983777455158d138471/messages",
          {
            method: "POST",
        ...
        console.error("Failed to notify base44 agent of user unban:", apiError);
      }
    }
    ...
    return {
      status: "error",
      message: error instanceof Error ? error.message : "Unable to unban user.",
    };
  }
}
